...
class Allloginlogs {
    public function getList() {
        ...
        $search_string = "1=1";
        $columns_list = "log_id,user_name,user_email,platform,browser_name,created_at";
        $token = "";
        $success = 1;
        $error = 0;
        $status = 200;
        ...
        if(isset($_REQUEST['search_text']) && $_REQUEST['search_text'] != ''){
            ...
            $search_string .= " AND (`user_email` LIKE '%" . addslashes($search_text) . "%' OR `user_name` LIKE '%" . addslashes($search_text) . "%') ";
        ...
        //print_r($item_alias);exit;
        //print_r($_SESSION[$back_session_name][$item_alias]);
        //exit;
        //echo $item_alias;
        $sql = "SELECT * FROM `login_logs` WHERE $search_string ORDER BY $sort_by $sort_type LIMIT $start,$records_per_page";
        //echo $sql;exit;
        $arrData = $cf->getData($sql);

        $sqlCount = "SELECT * FROM `login_logs` WHERE $search_string ";
        ...
        if(isset($arrData) && !empty($arrData)){
            foreach($arrData as $data) {
                $edit_url = $url."allitemlogs/form/?id=".$data['log_id'];
                $string .= '<tr>
                <td>'.$data['log_id'].'</td>
