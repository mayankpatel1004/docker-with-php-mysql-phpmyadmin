...
class Allsubscribers {
    public function getList() {
        ...
        $sql = "SELECT * FROM `subscriber` WHERE 1=1 $search_string AND deleted_status = 'N' ORDER BY $sort_by $sort_type LIMIT $start,$records_per_page";
        //echo $sql;exit;
        $arrData = $cf->getData($sql);

        $sqlCount = "SELECT * FROM `subscriber` WHERE 1=1 $search_string AND deleted_status = 'N'";
        ...
        if(isset($arrData) && !empty($arrData)){
            foreach($arrData as $data) {
                $edit_url = $url.$item_alias."/form/?id=".$data['subscriber_id'];
                ...
                $string .= '<tr>
                <td><input type="checkbox" name="chk[]" value="'.$data['subscriber_id'].'" class="form-control chkbox checkbox"></td>
                <td><a href="'.$edit_url.'">'.$data['email_address'].'</a></td>';
                $string .='
