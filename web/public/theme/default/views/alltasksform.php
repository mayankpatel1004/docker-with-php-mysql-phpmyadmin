            ...
            <div class="card-body">
                ...
                <form class="form-sample" action="<?php echo $save_url ; ?>" id="ajax-upload" method="post" enctype="multipart/form-data">
                    ...
                    if(isset($_SESSION[$back_session_name]['role_id']) && $_SESSION[$back_session_name]['role_id'] < 4):
                        if ($handle = opendir(ITEMS_PATH)) {
                            $task_idd = 0;
                            $number_length = 2;
                            if(isset($arrOnedata['task_id']) && $arrOnedata['task_id'] > 0) {
                                $task_idd = $arrOnedata['task_id'];
                                $number_length = strlen($task_idd);
                                $number_length = $number_length + 5;
                            }
                            while (false !== ($entry = readdir($handle))) {
                                if ($entry != "." && $entry != "..") {
                                    if(substr($entry,0,$number_length) == "task_".$task_idd){
                                        ?>
                                        <a href="<?php echo ITEMS_URL.$entry;?>" target="_blank">View Previous Attachment</a> | 
                                        <?php
                                    }
                                }
                            }
