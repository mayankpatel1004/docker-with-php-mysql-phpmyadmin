...
<div class="main-panel">
        <div class="content-wrapper">
			<form name="report" class="reportform" method="post" action="<?php echo $url.'reportitem/itemReportExport';?>">
			<div class="card">
				<div class="card-body">
				<?php
		...
		<div class="row">
				...
				<select name="item_title" class="form-control">
					<option value="">Select Title</option>
						...
						if(isset($arrItemtitle) && !empty($arrItemtitle)) {
							...
							?>
								<option value="<?php echo $data['item_id'];?>"><?php echo ucfirst($data['item_title']);?></option>
							<?php
							}
						}
						?>
			...
			<div class="col-md-2">
				<select name="admin_module" class="form-control">
					<option value="">Admin Module</option>
						<?php
						if(isset($arrAdminModules) && !empty($arrAdminModules)) {
							foreach($arrAdminModules as $data) {
							?>
								<option value="<?php echo $data['item_id'];?>"><?php echo ucfirst($data['item_title']);?></option>
							<?php
							}
				...
				<select name="deleted_status" class="form-control">
					<option value="">Deleted Item</option>
						...
						if(isset($arrDeletedItems) && !empty($arrDeletedItems)) {
							...
							?>
								<option value="<?php echo $data['item_id'];?>"><?php echo $data['item_title'];?></option>
							<?php
							}
						}
						?>
		...
		<div class="col-md-2">
				<select name="robots" class="form-control">
					<option value="">Robots</option>
						<?php			
						if(isset($arrRobots) && !empty($arrRobots)) {
							foreach($arrRobots as $data) {
							?>
								<option value="<?php echo $data['robots'];?>"><?php echo $data['robots'];?></option>
							<?php
							}
						}
						?>
